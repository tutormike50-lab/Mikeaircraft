  ...
  case "$answer" in
    ...
    *)
      if [[ $CREDENTIALS_ONLY -eq 1 ]]; then
        echo "Kept the existing credentials. Nothing changed."
        exit 0
      fi
      echo "Keeping the existing credentials and updating the bridge installation."
...
if [[ $UPDATE_CREDENTIALS -eq 1 ]]; then
  echo "Paste the NEW Vercel bridge token, then press Enter. Input is being accepted but hidden."
  IFS= read -r -s bridge_token
  echo
  bridge_token="${bridge_token#"${bridge_token%%[![:space:]]*}"}"
  bridge_token="${bridge_token%"${bridge_token##*[![:space:]]}"}"
  ...
  chmod 0600 "$TEMP_ENV"
  {
    ...
    printf 'MIKEAIRCRAFT_PI_BRIDGE_LOCK=%s\n' "$(quote_env_value '/run/mikeaircraft/pi-bridge.lock')"
  } >"$TEMP_ENV"
