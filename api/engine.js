module.exports = async function handler(req, res) {
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.setHeader("Cache-Control", "no-store");

  const html = `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">

  <meta
    name="viewport"
    content="width=device-width, initial-scale=1.0"
  >

  <title>MikeAircraft Engine Monitor</title>

  <style>
    * {
      box-sizing: border-box;
    }

    body {
      margin: 0;
      background: #0c1119;
      color: #ffffff;
      font-family: Arial, Helvetica, sans-serif;
    }

    .header {
      background:
        linear-gradient(
          90deg,
          #071d34,
          #00558f,
          #071d34
        );

      border-bottom:
        1px solid #298dcc;

      padding: 22px 30px;
    }

    .header h1 {
      margin: 0;
      font-size: 27px;
    }

    .header p {
      margin: 7px 0 0;
      color: #a9cee8;
    }

    .container {
      max-width: 980px;
      margin: auto;
      padding: 28px;
    }

    .card {
      background: #151d29;
      border: 1px solid #29384b;
      border-radius: 12px;
      overflow: hidden;
      margin-bottom: 22px;
    }

    .cardTitle {
      padding: 14px 18px;
      background: #101722;
      border-bottom: 1px solid #29384b;
      font-size: 14px;
      font-weight: bold;
      letter-spacing: 1px;
      color: #a8d9ff;
    }

    .row {
      min-height: 48px;
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 20px;
      padding: 10px 18px;
      border-bottom: 1px solid #273240;
    }

    .row:last-child {
      border-bottom: none;
    }

    .label {
      color: #b8c4d2;
    }

    .value {
      font-weight: bold;
      text-align: right;
    }

    .good {
      color: #57dc91;
    }

    .bad {
      color: #ff6e6e;
    }

    .waiting {
      color: #ffd65a;
    }

    .blue {
      color: #7dc9ff;
    }

    .big {
      font-size: 24px;
    }

    .airportBox {
      display: flex;
      align-items: center;
      gap: 12px;
    }

    select {
      background: #0b253a;
      color: white;
      border: 1px solid #387ba6;
      border-radius: 6px;
      padding: 8px 12px;
      font-size: 14px;
    }

    #errorBox {
      display: none;
      background: #431f26;
      border: 1px solid #843e49;
      border-radius: 10px;
      color: #ffc0c6;
      padding: 15px;
      margin-bottom: 20px;
    }

    .footer {
      color: #74879a;
      font-size: 12px;
      text-align: center;
      margin-top: 18px;
    }

    .pulse {
      display: inline-block;
      width: 9px;
      height: 9px;
      border-radius: 50%;
      background: #57dc91;
      margin-right: 7px;
      box-shadow: 0 0 8px #57dc91;
    }

    .aircraftBox {
      padding: 16px 18px;
    }

    .aircraftTitle {
      font-size: 18px;
      font-weight: bold;
      color: #ffffff;
      margin-bottom: 8px;
    }

    .aircraftDetails {
      color: #b8c4d2;
      line-height: 1.7;
      font-size: 14px;
    }

    .state {
      display: inline-block;
      padding: 4px 8px;
      border-radius: 5px;
      background: #0b253a;
      color: #7dc9ff;
      font-weight: bold;
      margin-left: 6px;
    }

    .none {
      color: #74879a;
    }
  </style>
</head>

<body>

  <div class="header">
    <h1>✈ MikeAircraft Engine Monitor</h1>
    <p>
      Engine v2 • State Tracking Development Dashboard
    </p>
  </div>

  <div class="container">

    <div id="errorBox"></div>

    <div class="card">

      <div class="cardTitle">
        ENGINE STATUS
      </div>

      <div class="row">
        <span class="label">Airport</span>

        <div class="airportBox">
          <select id="airport">
            <option value="PRG">PRG — Prague</option>
            <option value="LHR">LHR — Heathrow</option>
            <option value="FRA">FRA — Frankfurt</option>
            <option value="AMS">AMS — Amsterdam</option>
            <option value="CDG">CDG — Paris CDG</option>
            <option value="MAN">MAN — Manchester</option>
          </select>
        </div>
      </div>

      <div class="row">
        <span class="label">Engine</span>
        <span id="engine" class="value waiting">
          WAITING
        </span>
      </div>

      <div class="row">
        <span class="label">Version</span>
        <span id="version" class="value">
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">Stage</span>
        <span id="stage" class="value blue">
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">ADS-B Source</span>
        <span id="source" class="value">
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">Redis Memory</span>
        <span id="redis" class="value waiting">
          WAITING
        </span>
      </div>

    </div>

    <div class="card">

      <div class="cardTitle">
        LIVE TRACKING
      </div>

      <div class="row">
        <span class="label">
          Aircraft received
        </span>

        <span
          id="aircraft"
          class="value big"
        >
          0
        </span>
      </div>

      <div class="row">
        <span class="label">
          Movement histories stored
        </span>

        <span
          id="histories"
          class="value big"
        >
          0
        </span>
      </div>

      <div class="row">
        <span class="label">
          Last successful update
        </span>

        <span
          id="lastUpdate"
          class="value"
        >
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">
          Update interval
        </span>

        <span class="value good">
          5 seconds
        </span>
      </div>

    </div>

    <div class="card">

      <div class="cardTitle">
        MEMORY HEALTH
      </div>

      <div class="row">
        <span class="label">
          Redis connected
        </span>

        <span
          id="redisConnected"
          class="value"
        >
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">
          Redis read
        </span>

        <span
          id="read"
          class="value"
        >
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">
          Redis write
        </span>

        <span
          id="write"
          class="value"
        >
          ---
        </span>
      </div>

      <div class="row">
        <span class="label">
          Memory error
        </span>

        <span
          id="memoryError"
          class="value"
        >
          None
        </span>
      </div>

    </div>

    <div class="card">

      <div class="cardTitle">
        AIRCRAFT STATES
      </div>

      <div class="row">
        <span class="label">On final</span>
        <span id="stateFinal" class="value">0</span>
      </div>

      <div class="row">
        <span class="label">Approaching</span>
        <span id="stateApproaching" class="value">0</span>
      </div>

      <div class="row">
        <span class="label">Takeoff roll</span>
        <span id="stateTakeoff" class="value">0</span>
      </div>

      <div class="row">
        <span class="label">Airborne departure</span>
        <span id="stateAirborneDeparture" class="value">0</span>
      </div>

      <div class="row">
        <span class="label">Departing</span>
        <span id="stateDeparting" class="value">0</span>
      </div>

      <div class="row">
        <span class="label">Ground / taxi</span>
        <span id="stateGround" class="value">0</span>
      </div>

      <div class="row">
        <span class="label">Other airborne</span>
        <span id="stateAirborne" class="value">0</span>
      </div>

    </div>

    <div class="card">

      <div class="cardTitle">
        NEXT ARRIVAL
      </div>

      <div
        id="arrivalBox"
        class="aircraftBox"
      >
        <div class="none">
          No arrival currently selected
        </div>
      </div>

    </div>

    <div class="card">

      <div class="cardTitle">
        NEXT DEPARTURE
      </div>

      <div
        id="departureBox"
        class="aircraftBox"
      >
        <div class="none">
          No departure currently selected
        </div>
      </div>

    </div>

    <div class="footer">
      MikeAircraft Engine v2 • Development Monitor
    </div>

  </div>

<script>
  const UPDATE_INTERVAL = 5000;

  let busy = false;

  function setStatus(id, text, good) {
    const element =
      document.getElementById(id);

    element.textContent = text;

    element.className =
      "value " +
      (
        good === true
          ? "good"
          : good === false
            ? "bad"
            : "waiting"
      );
  }

  function safe(value, fallback) {
    if (
      value === null ||
      value === undefined ||
      value === ""
    ) {
      return fallback;
    }

    return value;
  }

  function aircraftName(ac) {
    if (!ac) {
      return "Unknown aircraft";
    }

    return (
      ac.callsign ||
      ac.registration ||
      ac.id ||
      "Unknown aircraft"
    );
  }

  function renderAircraft(
    elementId,
    ac
  ) {
    const box =
      document.getElementById(
        elementId
      );

    if (!ac) {
      box.innerHTML =
        '<div class="none">' +
        'No aircraft currently selected' +
        '</div>';

      return;
    }

    const title =
      aircraftName(ac);

    const registration =
      safe(
        ac.registration,
        "Unknown registration"
      );

    const type =
      safe(
        ac.type,
        "Unknown type"
      );

    const runway =
      safe(
        ac.runway,
        "Unknown"
      );

    const altitude =
      ac.altitude !== null &&
      ac.altitude !== undefined
        ? ac.altitude + " ft"
        : "Unknown";

    const speed =
      ac.speed !== null &&
      ac.speed !== undefined
        ? ac.speed + " kt"
        : "Unknown";

    const distance =
      ac.distanceKm !== null &&
      ac.distanceKm !== undefined
        ? ac.distanceKm + " km"
        : "Unknown";

    const confidence =
      ac.confidence !== null &&
      ac.confidence !== undefined
        ? ac.confidence + "%"
        : "Unknown";

    box.innerHTML =
      '<div class="aircraftTitle">' +
      title +
      '<span class="state">' +
      safe(ac.state, "UNKNOWN") +
      '</span>' +
      '</div>' +

      '<div class="aircraftDetails">' +
      'Registration: ' +
      registration +
      '<br>' +

      'Type: ' +
      type +
      '<br>' +

      'Runway: ' +
      runway +
      '<br>' +

      'Distance: ' +
      distance +
      '<br>' +

      'Altitude: ' +
      altitude +
      '<br>' +

      'Speed: ' +
      speed +
      '<br>' +

      'Confidence: ' +
      confidence +
      '</div>';
  }

  async function updateMonitor() {

    if (busy) {
      return;
    }

    busy = true;

    const airport =
      document.getElementById(
        "airport"
      ).value;

    try {

      const response =
        await fetch(
          "/api/engine?airport=" +
          encodeURIComponent(airport) +
          "&t=" +
          Date.now(),
          {
            cache: "no-store"
          }
        );

      const text =
        await response.text();

      let data;

      try {
        data =
          JSON.parse(text);
      }
      catch {
        throw new Error(
          "Engine returned non-JSON data"
        );
      }

      if (!response.ok || !data.ok) {
        throw new Error(
          data.error ||
          "Engine request failed"
        );
      }

      const traffic =
        data.traffic || {};

      const memory =
        data.memory || {};

      const intelligence =
        data.intelligence || {};

      const states =
        intelligence.stateCounts || {};

      document.getElementById(
        "engine"
      ).innerHTML =
        '<span class="pulse"></span>LIVE';

      document.getElementById(
        "engine"
      ).className =
        "value good";

      document.getElementById(
        "version"
      ).textContent =
        data.version || "---";

      document.getElementById(
        "stage"
      ).textContent =
        data.stage || "---";

      document.getElementById(
        "source"
      ).textContent =
        traffic.source || "Unknown";

      document.getElementById(
        "aircraft"
      ).textContent =
        traffic.trackedCount ??
        traffic.rawCount ??
        0;

      document.getElementById(
        "histories"
      ).textContent =
        memory.trackedHistories ?? 0;

      setStatus(
        "redis",
        memory.redisConnected
          ? "CONNECTED"
          : "NOT CONNECTED",
        memory.redisConnected
      );

      setStatus(
        "redisConnected",
        memory.redisConnected
          ? "YES"
          : "NO",
        memory.redisConnected
      );

      setStatus(
        "read",
        memory.readOK
          ? "OK"
          : "FAILED",
        memory.readOK
      );

      setStatus(
        "write",
        memory.writeOK
          ? "OK"
          : "FAILED",
        memory.writeOK
      );

      const memoryError =
        document.getElementById(
          "memoryError"
        );

      if (memory.error) {
        memoryError.textContent =
          memory.error;

        memoryError.className =
          "value bad";
      }
      else {
        memoryError.textContent =
          "None";

        memoryError.className =
          "value good";
      }

      document.getElementById(
        "stateFinal"
      ).textContent =
        states.ON_FINAL || 0;

      document.getElementById(
        "stateApproaching"
      ).textContent =
        states.APPROACHING || 0;

      document.getElementById(
        "stateTakeoff"
      ).textContent =
        states.TAKEOFF_ROLL || 0;

      document.getElementById(
        "stateAirborneDeparture"
      ).textContent =
        states.AIRBORNE_DEPARTURE || 0;

      document.getElementById(
        "stateDeparting"
      ).textContent =
        states.DEPARTING || 0;

      document.getElementById(
        "stateGround"
      ).textContent =
        (
          (states.GROUND || 0) +
          (states.LINE_UP_OR_TAXI || 0) +
          (states.LANDED || 0)
        );

      document.getElementById(
        "stateAirborne"
      ).textContent =
        states.AIRBORNE || 0;

      renderAircraft(
        "arrivalBox",
        intelligence.nextArrival
      );

      renderAircraft(
        "departureBox",
        intelligence.nextDeparture
      );

      document.getElementById(
        "lastUpdate"
      ).textContent =
        new Date().toLocaleTimeString();

      document.getElementById(
        "errorBox"
      ).style.display =
        "none";

    }
    catch (error) {

      setStatus(
        "engine",
        "ERROR",
        false
      );

      const box =
        document.getElementById(
          "errorBox"
        );

      box.textContent =
        "Engine error: " +
        error.message;

      box.style.display =
        "block";
    }
    finally {
      busy = false;
    }
  }

  document.getElementById(
    "airport"
  ).addEventListener(
    "change",
    updateMonitor
  );

  updateMonitor();

  setInterval(
    updateMonitor,
    UPDATE_INTERVAL
  );
</script>

</body>
</html>
`;

  return res.status(200).send(html);
};
