...
async function lookupAirlabsRoute(callsign, airportCode, lineage) {
  ...
  if (!result.ok || !flight) return { route:null, source:"airlabs", error:result.error || "No AirLabs flight" };
  if (!flightMatchesAirport(flight, airportCode, lineage)) {
    return { route:null, source:"airlabs", error:"AirLabs route conflicts with watched airport" };
  }
  ...
  if (!origin || !destination) return { route:null, source:"airlabs", error:"Could not resolve AirLabs airport coordinates" };
