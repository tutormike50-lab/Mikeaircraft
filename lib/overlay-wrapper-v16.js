...
module.exports = async function handler(req, res) {
  ...
  res.send = function patchedSend(body) {
    ...
    html = html.replace(
      "</style>",
      ".telemetry-strip.telemetry-red .metric-value{color:#ff4d4d!important}.telemetry-strip.telemetry-green .metric-value{color:#55e889!important}.mikeaircraft-logo{position:absolute;top:3.5vh;left:3vw;z-index:18;display:flex;align-items:center;gap:8px;pointer-events:none;filter:drop-shadow(0 2px 5px rgba(0,0,0,.8));font-family:Arial,Helvetica,sans-serif;font-size:clamp(15px,1.15vw,22px);font-weight:900;letter-spacing:.7px;line-height:1}.mikeaircraft-logo .plane{font-size:.9em;color:#67ddff;transform:rotate(-8deg)}.mikeaircraft-logo .mike{color:#fff}.mikeaircraft-logo .aircraft{color:#67ddff}</style>"
    );

    html = html.replace(
      '<div class="overlay">',
      '<div class="overlay"><div class="mikeaircraft-logo"><span class="plane">✈</span><span><span class="mike">MIKE</span><span class="aircraft">AIRCRAFT</span></span></div>'
    );
