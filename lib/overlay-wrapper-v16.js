...
module.exports = async function handler(req, res) {
  ...
  res.send = function patchedSend(body) {
    ...
    // Highlight live telemetry values (distance, altitude, speed and runway)
    // in broadcast red while leaving the metric labels subdued.
    html = html.replace(
      "</style>",
      ".telemetry-strip .metric-value{color:#ff4d4d!important}</style>"
    );
