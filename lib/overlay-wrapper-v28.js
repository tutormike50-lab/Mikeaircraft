...
.ma-css-radar-dot.ground:not(.current) .ma-css-radar-label{color:#b9e8ff;text-shadow:-1px -1px 2px #061018,1px 1px 2px #061018,0 0 6px rgba(109,198,255,.7)}
.ma-css-radar-thumb{position:absolute;left:14px;top:11px;width:76px;height:48px;object-fit:cover;border-radius:6px;border:1px solid rgba(255,222,119,.9);background:#06131c;box-shadow:0 4px 13px rgba(0,0,0,.72),0 0 9px rgba(255,202,87,.55);opacity:0;transform:scale(.82);transform-origin:top left;transition:opacity .15s ease,transform .15s ease;pointer-events:none}
.ma-css-radar-thumb.left{left:auto;right:14px;transform-origin:top right}
.ma-css-radar-dot.current.sweep-hit .ma-css-radar-thumb.ready{opacity:1;transform:scale(1)}
...
showMain=function(current){
 ...
 window.__maCurrentRadarPhoto=current&&current.available&&current.aircraft?(current.aircraft.thumbnail||current.aircraft.photo||null):null;
 ...
 function renderDots(){
  ...
  for(let i=0;i<radarAircraft.length;i++){
   ...
   if(!dot){
    ...
    thumb.className="ma-css-radar-thumb";
    thumb.alt="";
   ...
   const label=dot.firstChild,thumb=dot.lastChild,placeLeft=left>63;
   ...
   const photo=current?window.__maCurrentRadarPhoto:null;
   thumb.className="ma-css-radar-thumb"+(placeLeft?" left":"")+(photo?" ready":"");
   if(photo&&thumb.getAttribute("src")!==photo)thumb.setAttribute("src",photo);
   if(!photo)thumb.removeAttribute("src");
  }
