...
.ma-ribbon-aircraft{
 ...
 height:76px;
 color:rgba(151,228,255,.3);
 opacity:0;
 transform:translate(-50%,-50%);
 transition:opacity .35s ease;
 pointer-events:none;
}
.ma-ribbon-aircraft.visible{opacity:1}
.ma-ribbon-aircraft .motion-track{position:absolute;inset:0;width:100%;height:100%;overflow:visible;fill:none;stroke:rgba(113,217,255,.24);stroke-width:2;stroke-linecap:round;stroke-dasharray:6 8}
.ma-ribbon-aircraft .motion-track path{display:none}
.ma-ribbon-aircraft.arrival .arrival-track{display:block}
.ma-ribbon-aircraft.departure .departure-track{display:block}
.ma-ribbon-aircraft .plane-icon{position:absolute;left:50%;top:50%;width:64px;height:58px;fill:currentColor;filter:drop-shadow(0 2px 7px rgba(81,205,255,.22))}
.ma-ribbon-aircraft.arrival .plane-icon{transform:translate(-50%,-50%) rotate(7deg)}
.ma-ribbon-aircraft.departure .plane-icon{transform:translate(-50%,-50%) rotate(-7deg)}
.main-ribbon.moment-active .ma-ribbon-aircraft{opacity:0}
...
showMain=function(current){
 const currentKey=current&&current.available?identityKey(current):null;
 const aircraftGraphic=document.querySelector(".ma-ribbon-aircraft");
 if(aircraftGraphic){
  const lineage=String(current&&current.movement&&current.movement.lineage||"").toUpperCase();
  aircraftGraphic.className="ma-ribbon-aircraft"+(current&&current.available?" visible":"")+(lineage==="ARRIVAL"?" arrival":lineage==="DEPARTURE"?" departure":"");
 }
 if(window.__maCompletedMomentAircraftKey&&currentKey===window.__maCompletedMomentAircraftKey){
  const lower=document.getElementById("lowerThird");
  if(lower)lower.classList.remove("visible");
  return;
 }
 if(currentKey!==window.__maCompletedMomentAircraftKey)window.__maCompletedMomentAircraftKey=null;
 __maShowMainBeforeMomentClose(current);
 const status=document.getElementById("status");
 if(status&&String(status.textContent||"").trim().toUpperCase()==="ON FINAL")status.textContent="FINAL APPROACH";
};
...
(function addRibbonAircraftGraphic(){
 const ribbon=document.querySelector(".main-ribbon");
 if(!ribbon||ribbon.querySelector(".ma-ribbon-aircraft"))return;
 const graphic=document.createElement("div");
 graphic.className="ma-ribbon-aircraft";
 graphic.setAttribute("aria-hidden","true");
 graphic.innerHTML='<svg class="motion-track" viewBox="0 0 250 76"><path class="arrival-track" d="M8 17 C72 20 132 39 242 61"/><path class="departure-track" d="M8 61 C83 59 151 39 242 15"/></svg><svg class="plane-icon" viewBox="0 0 576 512"><path d="M482.3 192c34.2 0 93.7 29 93.7 64 0 36-59.5 64-93.7 64H365.7L265.2 495.9c-5.7 10-16.3 16.1-27.8 16.1h-56.2c-10.6 0-18.3-10.2-15.4-20.4l49-171.6H112l-43.2 57.6c-3 4-7.8 6.4-12.8 6.4H14c-7.8 0-14-6.3-14-14 0-1.3.2-2.6.5-3.9L32 256 .5 145.9c-.4-1.3-.5-2.6-.5-3.9 0-7.8 6.3-14 14-14h42c5 0 9.8 2.4 12.8 6.4L112 192h102.9l-49-171.6C162.9 10.2 170.6 0 181.2 0h56.2c11.5 0 22.1 6.2 27.8 16.1L365.7 192h116.6z"/></svg>';
 ribbon.appendChild(graphic);
})();
