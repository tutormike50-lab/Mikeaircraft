...
window.__maWelcomedAircraftKey=window.__maWelcomedAircraftKey||null;
const __maShowMainBeforeWelcomeClose=showMain;
showMain=function(current){
 const currentKey=current&&current.available?identityKey(current):null;
 if(window.__maWelcomedAircraftKey&&currentKey===window.__maWelcomedAircraftKey){
  const lower=document.getElementById("lowerThird");
  if(lower)lower.classList.remove("visible");
  return;
 }
 if(currentKey!==window.__maWelcomedAircraftKey)window.__maWelcomedAircraftKey=null;
 __maShowMainBeforeWelcomeClose(current);
};
maShowMoment=function(text,kind,key){
 ...
 if(kind==="arrival")window.__maWelcomedAircraftKey=String(key).replace(/\|WELCOME$/,"");
...
(function addYoloSafeRadar(){
 ...
 function renderDots(){
  ...
  for(let i=0;i<radarAircraft.length;i++){
   ...
   label.className="ma-css-radar-label"+(left>63?" left":"");
   label.textContent=current?String((document.getElementById("flight")&&document.getElementById("flight").textContent)||a.callsign||a.registration||"CURRENT").trim():"";
  }
