...
(function addYoloSafeRadar(){
 ...
 const sweepArm=face.querySelector(".sweep");
 if(sweepArm)sweepArm.style.animationDelay="-"+(Date.now()%8000)+"ms";
 function updateSweepGlow(){
  const sweep=(Date.now()%8000)/8000*Math.PI*2;
  const keys=Object.keys(dots);
  for(let i=0;i<keys.length;i++){
...
(function syncPrgTowerWithRadarSweep(){
 const badge=document.querySelector(".prg-tower-badge");
 if(!badge)return;
 const towerBearing=Math.PI/4;
 function frame(){
  let sweep;
  if(document.querySelector(".ma-css-radar"))sweep=(Date.now()%8000)/8000*Math.PI*2;
  else{
   try{sweep=Number(radarSweepAngle)*Math.PI/180}catch(error){sweep=NaN}
  }
  if(Number.isFinite(sweep)){
   const difference=Math.abs(Math.atan2(Math.sin(sweep-towerBearing),Math.cos(sweep-towerBearing)));
   badge.classList.toggle("sweep-hit",difference<0.13);
  }else badge.classList.remove("sweep-hit");
