...
.prg-tower-badge.sweep-lit{
 border-color:#74ffc1;
 background:rgba(8,75,57,.88);
 box-shadow:0 0 8px rgba(116,255,193,.85),0 0 22px rgba(72,255,181,.65);
}
.prg-tower-badge.sweep-lit svg{stroke:#74ffc1;filter:drop-shadow(0 0 4px rgba(116,255,193,.95))}
.prg-tower-badge.sweep-lit .prg-tower-code{color:#74ffc1;text-shadow:0 0 7px rgba(116,255,193,.95)}
.prg-tower-badge svg{
 width:25px;
 height:25px;
 display:block;
...
(function addPrgTowerBadge(){
 ...
 function syncTowerFlash(){
  const sweep=(Number(radarSweepAngle)||0)*Math.PI/180;
  const difference=Math.abs(Math.atan2(Math.sin(sweep-targetAngle),Math.cos(sweep-targetAngle)));
  badge.classList.toggle("sweep-lit",difference<0.12);
  requestAnimationFrame(syncTowerFlash);
 }
 requestAnimationFrame(syncTowerFlash);
})();
</script></body>`);
