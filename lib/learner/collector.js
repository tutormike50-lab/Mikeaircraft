...
async function fetchAirport(airport){
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    if(r.ok){const j=await r.json();return Array.isArray(j.ac)?j.ac:[];}
    if((r.status===420||r.status===429)&&attempt===0){
      const retryAfter=Number(r.headers.get("retry-after"));
      await delay(Number.isFinite(retryAfter)&&retryAfter>0?Math.min(retryAfter*1000,5000):1500);
      continue;
    }
    throw new Error(`${SOURCE} HTTP ${r.status}`);
  }
  return [];
}
async function fetchAirportsPolite(airports,errors){
  const results=[];
  for(let i=0;i<airports.length;i++){
    const airport=airports[i];
    try{results.push({airport,aircraft:await fetchAirport(airport)});}
    catch(e){errors.push(`${airport.code}:${e.message}`);results.push({airport,aircraft:[]});}
    if(i<airports.length-1)await delay(FETCH_SPACING_MS);
  }
  return results;
}
...
async function runCollector(){
  ...
  try{
    ...
    const fetched=await fetchAirportsPolite(airports,errors);
