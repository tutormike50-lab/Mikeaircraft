...
async function runCollector(){
  ...
  try{
    ...
    const fetched=await Promise.all(airports.map(async airport=>{
      try{return {airport,aircraft:await fetchAirport(airport)};}
      catch(e){errors.push(`${airport.code}:${e.message}`);return {airport,aircraft:[]};}
    }));
