...
const { isCommercialTransport } = require("./commercial-aircraft-filter.js");
...
module.exports = async function handler(req, res) {
  ...
  const PRIVATE_TYPES = new Set([
    ...
    // Cessna Citation
    "C25A","C25B","C25C","C25M","C500","C501","C510","C525","C526","C550","C551","C55B","C560","C56X","C650","C680","C68A","C68B","C700","C750",
    // Cessna piston, utility and twin families — MikeAircraft is commercial-only
    "C120","C140","C150","C152","C162","C170","C172","C175","C177","C180","C182","C185","C188","C190","C195",
    "C205","C206","C207","C208","C210","C240","C303","C305","C310","C320","C335","C336","C337","C340","C350",
    "C401","C402","C404","C406","C411","C414","C421","C425","C441",
    "A185","C77R","C82R","P210","R172","R182","T206","T210","T303","T337","TR182","U206",
    // Gulfstream / Galaxy
  ...
  const NON_COMMERCIAL_OPERATORS = new Set([
    // Air Transport Europe — air ambulance/private charter rather than scheduled airline traffic
    "EAT"
  ]);
  ...
  function isPrivateOrBusiness(ac) {
    if (!ac) return false;
    if (!isCommercialTransport(ac)) return true;
    const type = clean(ac.type || ac.typeCode || ac.aircraft?.typeCode || ac.t);
    const callsignPrefix = clean(ac.callsign || ac.flight || ac.call).slice(0, 3);
    if (NON_COMMERCIAL_OPERATORS.has(callsignPrefix)) return true;
    ...
    if (/^C25[A-Z]$/.test(type)) return true;
    if (/^C55[A-Z]$/.test(type)) return true;
    if (/^C68[A-Z]$/.test(type)) return true;
    return false;
  }
  ...
  res.json = async function quietAirportJson(payload) {
    ...
    if (payload.traffic) {
      ...
      payload.traffic.nonCommercialAircraftFiltered = removedCount;
      payload.traffic.commercialOnly = true;
    ...
    if (!promotedRaw) {
      payload.intelligence.editorialLock = {
        ...
        commercialOnlyFilterEnabled: true,
