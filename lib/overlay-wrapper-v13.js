...
module.exports = async function handler(req, res) {
  ...
  res.send = function patchedSend(body) {
    ...
    // Do not blink a truthful LIVE MOVEMENT map off every refresh while waiting
    // for the auxiliary Engine request. Preserve it when it belongs to the same
    // CURRENT aircraft; only clear a stale map left by a previous aircraft.
    html = html.replace(
    ...
    const mapBelongsToCurrent=Boolean(currentKey&&lastRouteKey&&String(lastRouteKey).startsWith(currentKey+"|"));
    if(mapBelongsToCurrent){storyCard.classList.remove("mapless")}
    else{document.getElementById("routeMapCard").classList.remove("visible");routeAnimationActive=false;storyCard.classList.add("mapless")}
   }`
    );
