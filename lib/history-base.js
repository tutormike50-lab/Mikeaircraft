module.exports = async function handler(req, res) {
  ...
  try {
    const allowedAirports = ["PRG", "LHR", "FRA", "AMS", "CDG", "MAN", "ATL"];
    ...
    if (!redisURL || !redisToken) return res.status(500).json({ ok: false, error: "Redis environment variables unavailable" });
    ...
    if (req.method === "POST") {
      ...
      const entry = {
        time: Date.now(), iso: new Date().toISOString(), airport,
    ...
    if (mode === "audit" || mode === "data") {
      ...
      const entries = (raw || []).map(value => { try { return JSON.parse(value); } catch { return null; } }).filter(entry => entry && entry.time >= cutoff).reverse();
...
tick();setInterval(tick,5000);
    ...
    if (!stored) return res.status(200).json({ ok: true, airport, historyFound: false, trackedAircraft: 0, tracks: [] });

    const savedState = JSON.parse(stored);
    const storedTracks = savedState.tracks && typeof savedState.tracks === "object" ? savedState.tracks : {};
    const tracks = Object.entries(storedTracks).map(([id, track]) => {
      const samples = Array.isArray(track.samples) ? track.samples : [];
      const latest = samples.length ? samples[samples.length - 1] : null;
      const oldest = samples.length ? samples[0] : null;
      return { id, state: track.state || "UNKNOWN", confidence: track.confidence ?? null, reason: track.reason || null, stateSince: track.stateSince || null, lastSeen: track.lastSeen || null, sampleCount: samples.length, historySeconds: oldest && latest ? Math.round((latest.time - oldest.time) / 1000) : 0, latest, samples };
    }).sort((a, b) => (b.lastSeen || 0) - (a.lastSeen || 0));

    return res.status(200).json({ ok: true, engine: "MikeAircraft Engine v2", diagnostic: "Movement History", airport, historyFound: true, updatedAt: savedState.updatedAt || null, trackedAircraft: tracks.length, tracks });
  } catch (error) {
