...
async function buildTarget(target, role, { enrichHandler, routeHandler, headers = {}, airport = null }) {
  ...
  const [enrichResult, routeResult, aircraftInfo] = await Promise.all([
    ...
    callsign
      ? invokeHandler(routeHandler, { callsign, airport:airport || "", lineage:target.lineage || "" }, headers)
          .catch(() => ({ status:500, data:null }))
  ...
  if (routeData?.mikeaircraft_source) route.source = routeData.mikeaircraft_source;
