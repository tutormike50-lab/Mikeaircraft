...
// shortly before the main CURRENT ribbon ends.
...
@media(max-width:900px){.route-map-card{width:min(65vw,480px)!important;height:190px!important}.story-card{width:min(65vw,480px)!important;top:calc(3.5vh + 190px)!important}.story-card.mapless{top:3.5vh!important}}
    ...
    if (routeStart >= 0 && routeEnd > routeStart) {
      const newRoute = `function showRouteMap(t){
...
function infoPanelClosing(engineCurrent){
 if(!engineCurrent)return false;
 ...
 if(lineage==="DEPARTURE"&&state==="DEPARTING"&&Number.isFinite(distance)&&distance>=3)return true;
 return false
}
async function update(){
 ...
 try{
  ...
  const closing=infoPanelClosing(engine?.intelligence?.current);
  if(!current?.available||closing){
   ...
   if(sr.ok&&story.ok)showStory(story);else showStory(null)
  }
