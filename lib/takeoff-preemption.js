...
function arrivalCanYield(current) {
  ...
  const lineage = String(current.lineage || "").toUpperCase();
  ...
  if (lineage === "DEPARTURE") {
    if (state === "TAKEOFF_ROLL") return false;

    const distance = distanceKm(current);
    if (["AIRBORNE_DEPARTURE", "DEPARTING"].includes(state)) {
      return distance !== null && distance > 2.5;
    }

    // A confirmed takeoff must replace a stationary, taxiing or merely queued
    // departure left behind by a quiet-airport/editorial lock.
    return true;
  }
...
function isManualPriorityCandidate(aircraft, mode) {
  ...
  if (mode === "RUNWAY") {
    ...
    const alignment = finiteNumber(aircraft.runwayAlignment);
    const speed = finiteNumber(aircraft.speed) || 0;
    if (runwayDistance === null || runwayDistance > 0.24 || speed < 4) return false;
    if (state === "TAKEOFF_ROLL") return speed >= 35;
    if (state === "LINING_UP") return alignment !== null && alignment <= 30;
    return runwayDistance <= 0.12 && alignment !== null && alignment <= 20;
  }
