...
module.exports = async function handler(req, res) {
  ...
  res.send = function patchedSend(body) {
    ...
    const fallbackLine = `if(engine?.ok){radarAircraft=Array.isArray(engine.aircraft)?engine.aircraft:[];radarAirport={lat:Number(engine.airport?.lat),lon:Number(engine.airport?.lon)};if(!Number.isFinite(radarAirport.lat)||!Number.isFinite(radarAirport.lon))radarAirport=null;if(current?.available&&!closing&&!current.route?.found){showLocalMovementMap(current,engine,airport)}}`;
