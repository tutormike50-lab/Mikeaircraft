...
function view(s, now, saved = { yawDeg: 0, pitchDeg: 0 }) {
  ...
  return { connected, ready: connected && s.ready && !s.stopRequested,
    ...
    adjusting: Boolean(s?.adjusting), savedBoresight: saved,
    adjustment: s?.applied || { revision: 0, pan: 0, tilt: 0 },
    commandWindowUntil: now + COMMAND_MS, maxTrim: MAX_TRIM };
}
function transition(previous, body, now, saved = { yawDeg: 0, pitchDeg: 0 }) {
  ...
  if (action === 'open') {
    ...
    return { sessionId: body.sessionId, heartbeatAt: now, heartbeatSeq: 0,
      ...
      applied: { revision: 0, pan: 0, tilt: 0 }, adjusting: false,
      baseline: null, stopRequested: false };
  }
  ...
  if (action === 'start-adjust') {
    if (!s.ready || s.mode !== 'TRACKING' || s.stopRequested) fail('Controller is not ready for adjustment');
    if (s.command) fail('Waiting for the Pi to acknowledge the previous correction');
    s.adjusting = true; s.baseline = { yawDeg: saved.yawDeg, pitchDeg: saved.pitchDeg };
    s.applied = { revision: s.revision, pan: 0, tilt: 0 }; s.command = null;
    return s;
  }
  if (action === 'cancel-adjust') {
    if (!s.adjusting) fail('No adjustment session is active');
    s.adjusting = false; s.baseline = null;
    s.revision += 1;
    s.command = { revision: s.revision, pan: 0, tilt: 0, rebase: 'cancel', expiresAt: now + COMMAND_MS };
    return s;
  }
  if (action === 'save-adjust') {
    if (!s.adjusting) fail('No adjustment session is active');
    if (Math.abs(saved.yawDeg + s.applied.pan) > MAX_TRIM || Math.abs(saved.pitchDeg + s.applied.tilt) > MAX_TRIM) fail('Saved boresight limit exceeded');
    s.adjusting = false; s.baseline = null; s.saveBoresight = {
      yawDeg: saved.yawDeg + s.applied.pan, pitchDeg: saved.pitchDeg + s.applied.tilt
    };
    s.revision += 1;
    s.command = { revision: s.revision, pan: 0, tilt: 0, rebase: 'save', expiresAt: now + COMMAND_MS };
    return s;
  }
  if (action !== 'trim') fail('Unsupported action', 400);
  if (!s.adjusting) fail('Press START ADJUST before using the joystick');
