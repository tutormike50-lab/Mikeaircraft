...
function publicState(desiredValue, heartbeatValue, now = Date.now()) {
  ...
  return {
    ...
    piVersion: online && typeof heartbeat?.release?.serverCommit === "string" ? heartbeat.release.serverCommit : null,
    piReleaseId: online && typeof heartbeat?.release?.releaseId === "string" ? heartbeat.release.releaseId : null,
    bridgeHealth: online && heartbeat?.bridgeHealth === "HEALTHY" ? "HEALTHY" : "FAULT",
    trackerHealth: online && new Set(["HEALTHY", "IDLE", "FAULT"]).has(heartbeat?.trackerHealth) ? heartbeat.trackerHealth : "FAULT",
