...
async def main():
    print("REAL HOME POSITION PAN CALIBRATION", flush=True)
    ...
    print("PAN ONLY - TILT IS DISABLED. One aircraft, then return to Hornbach HOME.", flush=True)
    ...
    try:
        ...
        while selection is None:
            ...
            if candidate:
                try:
                    t = await asyncio.to_thread(real_local_target, candidate)
                except Exception:
                    t = None
                if t and t["distance"] <= LOCK_RANGE_KM:
                    ...
                    break
                if t:
                    print(f"CURRENT {t['callsign']} {t['distance']:.1f} km from CAMERA - waiting for 12.0 km", flush=True)
                else:
        ...
        print(f"LOCKED {selection['callsign']} {selection['state']} - tracking for {TRACK_SECONDS:.0f} seconds", flush=True)
        ...
        while time.monotonic() - track_start < TRACK_SECONDS:
            ...
            print(
                ...
                f"Hornbach offset {relative:+.1f} deg  {'ON_TARGET' if reached else 'CORRECTING'}",
                flush=True,
            )
        ...
        print("REAL POSITION PAN TEST:", type(exc).__name__, str(exc), flush=True)
    finally:
        try:
            await stop_motion()
        except Exception:
